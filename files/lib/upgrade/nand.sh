...
nand_upgrade_prepare_ubi() {
	...
	# ---- local patch: never erase the running rootfs unless the new one fits ----
	# Upstream removes kernel/rootfs/rootfs_data and then re-creates all three, in
	# that order. But ubirmvol on a UBIFS volume that is still mounted fails with
	# EBUSY, and rootfs_data backs /overlay, so it cannot be removed here. stage2
	# only unmounts /overlay when no config backup was requested (i.e. `sysupgrade
	# -n`); when config IS preserved the unmount is skipped, rootfs_data stays
	# pinned, and only the space freed by the old rootfs becomes available.
	# If the incoming rootfs is larger than the outgoing one, ubimkvol then fails
	# *after* rootfs has already been deleted, leaving the device unbootable.
	# So: work out up front whether the new rootfs can fit, and refuse before
	# touching anything if it cannot - a refused upgrade is recoverable, a
	# half-applied one is not.
	local _leb=126976
	local _old_root_leb=0
	[ -n "$root_ubivol" ] && _old_root_leb="$(cat /sys/class/ubi/$root_ubivol/reserved_ebs 2>/dev/null)"
	local _avail_leb="$(cat /sys/class/ubi/$root_ubidev/avail_eraseblocks 2>/dev/null)"
	local _need_leb=$(( (rootfs_length + _leb - 1) / _leb ))
	local _can_leb=$(( ${_old_root_leb:-0} + ${_avail_leb:-0} ))
	if [ -n "$rootfs_length" ] && [ "$_need_leb" -gt "$_can_leb" ]; then
		# Try to make room the way upstream intended to.
		[ -n "$data_ubivol" ] && ubirmvol /dev/$root_ubidev -N rootfs_data 2>/dev/null
		_avail_leb="$(cat /sys/class/ubi/$root_ubidev/avail_eraseblocks 2>/dev/null)"
		_can_leb=$(( ${_old_root_leb:-0} + ${_avail_leb:-0} ))
	fi
	if [ -n "$rootfs_length" ] && [ "$_need_leb" -gt "$_can_leb" ]; then
		echo "rootfs will not fit: need ${_need_leb} LEB, can reclaim ${_can_leb} LEB" >&2
		echo "rootfs_data could not be freed (still mounted). Re-run sysupgrade with -n," >&2
		echo "or free space in a ramdisk system. Nothing has been erased." >&2
		return 1
	fi

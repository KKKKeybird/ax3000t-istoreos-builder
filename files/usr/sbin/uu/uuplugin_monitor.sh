...
openwrt_download_url_init() {
    ...
    mips=$(echo "${MODEL}" | grep "mipseb")
    if [ "" != ${mips} ];then
        DOWNLOAD_URL="${DOWNLOAD_URL}openwrt-mipseb"
        return 0
    fi

    mips=$(echo "${MODEL}" | grep "mipsel")
    if [ "" != ${mips} ];then
...
steam_deck_download_url_init() {
    ...
    x86_64=$(echo "${MODEL}" | grep "x86_64")
    if [ "" != ${x86_64} ];then
        DOWNLOAD_URL="${DOWNLOAD_URL}${STEAM_DECK_PLUGIN}-x86_64"
        return 0
    fi
    return 1
}
...
asuswrt_download_url_init() {
    ...
    case "${model}" in
        ...
        EA6400)
            ...
            ;;
        MAP-AC1750)
            DOWNLOAD_URL="${DOWNLOAD_URL}asuswrt-merlin-mapac1750"
            return 0
            ;;
        *)
            if [ "${arm}" = "7" ];then
            ...
            if [ ! -z "${mips}" ];then
                DOWNLOAD_URL="${DOWNLOAD_URL}merlin-mipsel"
                return 0
            fi
            ;;
    esac
...
check_plugin_upgrade() {
    ...
    local latest_plugin_info=$(curl -L -s -k -H "Accept:text/plain" "${DOWNLOAD_URL}" || \
        ...
        wget -q -O - "${DOWNLOAD_URL}&output=text" || \
...
# check_plugin_upgrade  # disabled: keep vendor identity patch
